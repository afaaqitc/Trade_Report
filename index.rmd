...
main_dt<-
  main %>% datatable(class = "hover row-borders",
...
inner_join(wallet,by="Rounds") %>%
mutate(pct=Total/amount,Profit_Balance=round(cumsum(Total),2)) %>% 
select(- c(11) )%>%
select(Rounds,month_week,Saturday,Sunday,Monday,Tuesday,Wednesday,Thursday,Friday,Total,Profit_Balance,pct) 
...
month_week_totals_dt<-
  ...
  formatCurrency(columns = c(3:11),digits = 2) %>% formatPercentage(columns =12 ,digits = 2)
...
Order_Status_totals<-
qx %>% group_by(trade_status) %>% 
summarize(Trades=n(),Gain=sum(gain_n),Loss=sum(loss_n),
Profits=round(sum(net),2)) 

Order_Status_totals_dt<-
Order_Status_totals %>% adorn_totals("row")%>% 
