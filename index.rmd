...
Rounds = case_when(as.Date(opendate)>"2024-07-20" ~ "R2",as.Date(opendate)>'2024-08-01' ~ 'R3',TRUE ~ "R1"), hours =hour(as.POSIXct(opendate)),
...
Daily_Search<-
qx %>% select(Rounds,date,rate,type,income,amount.y,amount.x,net,trade_status,pairs,days,time_ranges) %>% rename(currencies=pairs,amount=amount.y) %>%arrange(date) %>%mutate() %>%   rename_with(.fn = toupper,.cols = everything()) 

Daily_Search_dt<-
Daily_Search %>% datatable(filter = "top",class="hover row-border", extensions = list(Buttons=TRUE,RowGroup=TRUE),options =list(dom="Bfrtip",buttons=c("csv","excel","pdf"),rowGroup=list(dataSrc=c(1)),columnDefs=list(list(className="dt-center",targets=1:11),list(visible=FALSE,targets=1)))) %>% formatCurrency(columns =c(5:8) ,digits = 2)
...
Order_Status_totals%>%rename_with(.fn = toupper,.cols = everything()) %>% adorn_totals("row")%>% 
datatable(width = "fit-content",class="hover row-border",extensions = list(Buttons=TRUE),
options = list(dom="Bfrtip",buttons=c("excel","csv","pdf"),
